...
namespace STS2RitsuLib.Settings.Patches
{
    ...
    /// <summary>
    ///     Reuses one <see cref="RitsuModSettingsSubmenu" /> per <see cref="NRunSubmenuStack" /> (pause / in-run UI).
    /// </summary>
    public class ModSettingsRunSubmenuStackPatch : IPatchMethod
    {
        private static readonly ConditionalWeakTable<NRunSubmenuStack, RitsuModSettingsSubmenu> Submenus = new();

        /// <inheritdoc />
        public static string PatchId => "ritsulib_mod_settings_submenu_run_stack";

        /// <inheritdoc />
        public static string Description => "Inject RitsuLib mod settings submenu into the in-run submenu stack";
        ...
        private static RitsuModSettingsSubmenu CreateSubmenu(NRunSubmenuStack stack)
        {
            var submenu = new RitsuModSettingsSubmenu
            {
                Visible = false,
