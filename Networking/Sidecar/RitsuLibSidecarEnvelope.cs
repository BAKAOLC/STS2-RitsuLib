...
namespace STS2RitsuLib.Networking.Sidecar
{
    ...
    {
        /// <summary>Result of parsing an on-wire envelope.</summary>
        public enum ParseOutcome
        {
            /// <summary>Parse succeeded.</summary>
            Ok,

            /// <summary>Packet shorter than the minimum header.</summary>
            TooSmall,

            /// <summary>Magic mismatch.</summary>
            BadMagic,

            /// <summary>Wire format version is zero or greater than <see cref="RitsuLibSidecarWire.SupportedWireFormatVersionMax" />.</summary>
            WireVersionUnsupported,

            /// <summary>Declared payload length invalid, gzip corrupt, or decompressed size over cap.</summary>
            PayloadLengthInvalid,

            /// <summary>Header extension length over cap.</summary>
            ExtensionLengthInvalid,

            /// <summary>Total packet length does not match header fields.</summary>
            TotalLengthMismatch,
        }

        private const uint KnownWireFlagsMask = (uint)RitsuLibSidecarWireFlags.PayloadGzip;

        /// <summary>Parses an envelope from a byte array backing store.</summary>
        ...
        /// <summary>Parses an envelope; <paramref name="backing" /> must be the same array as <paramref name="packet" /> spans.</summary>
        ...
        public static ParseOutcome TryParse(ReadOnlySpan<byte> packet, byte[] backing, out ParsedEnvelope parsed)
        {
            ...
            var o = RitsuLibSidecarWire.MagicLength;
            var wireVersion = BinaryPrimitives.ReadUInt16BigEndian(packet.Slice(o, 2));
            o += 2;
            var flagsRaw = BinaryPrimitives.ReadUInt32BigEndian(packet.Slice(o, 4));
            o += 4;
            var opcode = BinaryPrimitives.ReadUInt64BigEndian(packet.Slice(o, 8));
            o += 8;
            var payloadLen = BinaryPrimitives.ReadUInt32BigEndian(packet.Slice(o, 4));
            o += 4;
            var extLen = BinaryPrimitives.ReadUInt32BigEndian(packet.Slice(o, 4));
            o += 4;
            ...
            var total = o + extLen + payloadLen;
            if (total != packet.Length)
                return ParseOutcome.TotalLengthMismatch;

            var extMem = extLen == 0
                ? ReadOnlyMemory<byte>.Empty
                : new(backing, o, (int)extLen);

            o += (int)extLen;
            var rawPayload = new ReadOnlyMemory<byte>(backing, o, (int)payloadLen);
            ...
            bool gzipLogicalPayload)
        {
            ...
            var i = RitsuLibSidecarWire.MagicLength;
            BinaryPrimitives.WriteUInt16BigEndian(span.Slice(i, 2), wireFormatVersion);
            i += 2;
            BinaryPrimitives.WriteUInt32BigEndian(span.Slice(i, 4), (uint)flags);
            i += 4;
            BinaryPrimitives.WriteUInt64BigEndian(span.Slice(i, 8), opcode);
            i += 8;
            BinaryPrimitives.WriteUInt32BigEndian(span.Slice(i, 4), (uint)wirePayload.Length);
            i += 4;
            BinaryPrimitives.WriteUInt32BigEndian(span.Slice(i, 4), (uint)headerExtension.Length);
            i += 4;
            headerExtension.CopyTo(span[i..]);
            i += headerExtension.Length;
            wirePayload.CopyTo(span[i..]);
            return buffer;
        }

        /// <summary>Decoded header fields and logical payload.</summary>
        public readonly struct ParsedEnvelope
        {
            /// <summary>Creates a parsed envelope value.</summary>
            ...
            /// <summary>Wire format version from the packet.</summary>
            public ushort WireFormatVersion { get; }

            /// <summary>Decoded flags (unknown bits cleared).</summary>
            public RitsuLibSidecarWireFlags Flags { get; }

            /// <summary>64-bit opcode (from <see cref="RitsuLibSidecarOpcodes.For" /> or a framework constant).</summary>
            public ulong Opcode { get; }

            /// <summary>Opaque header extension; v1 senders typically use length 0.</summary>
            public ReadOnlyMemory<byte> HeaderExtension { get; }

            /// <summary>Logical payload (after optional gzip decompression).</summary>
