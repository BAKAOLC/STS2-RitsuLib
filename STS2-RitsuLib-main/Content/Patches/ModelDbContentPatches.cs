...
namespace STS2RitsuLib.Content.Patches
{
    ...
    {
        private static bool _androidLogged;
        ...
        public static void Postfix(ref IEnumerable<CharacterModel> __result)
        {
            ...
            if (OperatingSystem.IsAndroid() && !_androidLogged)
            {
                _androidLogged = true;
                PatchLog.For<AllCharactersPatch>().Info(
                    $"[AndroidCompat] ModelDb.AllCharacters appended {ModContentRegistry.GetModCharacters().Count()} mod character(s).");
            }
        }
    }
