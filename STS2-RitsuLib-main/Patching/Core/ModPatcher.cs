...
namespace STS2RitsuLib.Patching.Core
{
    ...
    public class ModPatcher(string patcherId, Logger logger, string patcherName = "")
    {
        ...
        private ModPatchResult ApplyPatch(ModPatchInfo modPatchInfo)
        {
            ...
            {
                logger.Info(
                    $"{_logPrefix}Applying patch '{modPatchInfo.Id}' " +
                    $"-> {modPatchInfo.TargetType.FullName}.{modPatchInfo.MethodName}"
                );
            ...
            DynamicPatchInfo dynamicPatchInfo)
        {
            ...
            {
                logger.Info(
                    $"{_logPrefix}Applying dynamic patch '{dynamicPatchInfo.Id}' " +
                    $"-> {dynamicPatchInfo.OriginalMethod.DeclaringType?.FullName}.{dynamicPatchInfo.OriginalMethod.Name}"
                );
