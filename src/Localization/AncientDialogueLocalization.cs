...
{
    /// <summary>
    ///     <para xml:lang="en">Loads ancient-event dialogue lines from localization tables and adds them to <c>AncientDialogueSet</c> instances for modded characters.</para>
    ...
    public static class AncientDialogueLocalization
    {
        ...
        /// <summary>
        ///     <para xml:lang="en">Builds the <c>{ancient}.talk.{character}.</c> localization-key prefix for an ancient event and character entry ID.</para>
        ...
        /// <summary>
        ///     <para xml:lang="en">Reads the contiguous, zero-based dialogue sequences for an ancient event and character from the <c>ancients</c> localization table.</para>
        ...
        /// <summary>
        ///     <para xml:lang="en">Reads dialogue sequences under <paramref name="baseKey" /> from <paramref name="locTable" />. Sequence and line indices must each be contiguous and start at zero; scanning stops at the first missing index.</para>
        ///     <para xml:lang="zh-CN">从 <paramref name="locTable" /> 读取 <paramref name="baseKey" /> 下的对话序列。序列索引和行索引均须从零开始且连续；遇到首个缺失索引时即停止扫描。</para>
        /// </summary>
        ...
        /// <summary>
        ///     <para xml:lang="en">Builds an <see cref="AncientDialogueSet" /> for a modded ancient event from the <c>ancients</c> localization table. It reads the first <c>{id}.talk.firstVisitEver.*</c> sequence, all <c>{id}.talk.ANY.*</c> sequences, and sequences for each base-game character.</para>
        ///     <para xml:lang="zh-CN">根据 <c>ancients</c> 本地化表为模组先古之民事件构建 <see cref="AncientDialogueSet" />。该方法读取首个 <c>{id}.talk.firstVisitEver.*</c> 序列、全部 <c>{id}.talk.ANY.*</c> 序列，以及每个原版角色的对话序列。</para>
        /// </summary>
        /// <remarks>
        ///     <para xml:lang="en">Characters registered in <see cref="ModContentRegistry" /> are intentionally omitted. RitsuLib's <c>PopulateLocKeys</c> prefix adds their entries once through <see cref="AppendCharacterDialogues" />.</para>
        ///     <para xml:lang="zh-CN">此处会有意跳过在 <see cref="ModContentRegistry" /> 中注册的角色；RitsuLib 的 <c>PopulateLocKeys</c> 前置补丁会通过 <see cref="AppendCharacterDialogues" /> 添加一次这些角色的条目。</para>
        /// </remarks>
        ...
        /// <summary>
        ///     <para xml:lang="en">Appends localization-defined dialogues for <paramref name="characters" /> to <paramref name="dialogueSet" /> for <paramref name="ancientEntry" />. Existing dialogues are retained; this method does not remove duplicates.</para>
        ///     <para xml:lang="zh-CN">将 <paramref name="characters" /> 在 <paramref name="ancientEntry" /> 下由本地化定义的对话追加到 <paramref name="dialogueSet" />。已有对话会被保留，且此方法不会移除重复项。</para>
        /// </summary>
        /// <returns>
