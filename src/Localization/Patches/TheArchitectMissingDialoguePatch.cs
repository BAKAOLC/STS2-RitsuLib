...
{
    /// <summary>
    ///     <para xml:lang="en">Prevents THE_ARCHITECT's <c>WinRun</c> path from dereferencing a null dialogue when no <see cref="AncientDialogue" /> can be resolved for a registered modded character.</para>
    ///     <para xml:lang="en">The fallback dialogue has an empty <see cref="AncientDialogue.Lines" /> collection, preserving the base game's no-dialogue options and UI path while providing an <c>EndAttackers</c> value for <c>WinRun</c>. A non-empty uninitialized dialogue would instead leave the room without buttons.</para>
    ///     <para xml:lang="en">This patch applies only when both the debug-compatibility master switch and the Ancient/THE_ARCHITECT compatibility setting are enabled. Otherwise, the base-game behavior, including a possible null-reference exception after PROCEED, remains unchanged.</para>
    ///     <para xml:lang="zh-CN">当已注册的模组角色无法解析出 <see cref="AncientDialogue" /> 时，避免 THE_ARCHITECT 的 <c>WinRun</c> 流程解引用空对话。</para>
    ///     <para xml:lang="zh-CN">回退对话的 <see cref="AncientDialogue.Lines" /> 集合为空，因此既能保留原版无对话时的选项和界面流程，又能为 <c>WinRun</c> 提供 <c>EndAttackers</c> 值。若使用包含未初始化台词的非空对话，房间反而会没有可用按钮。</para>
    ...
    internal class TheArchitectLoadDialogueMissingFallbackPatch : IPatchMethod
    {
        ...
        /// <summary>
        ///     <para xml:lang="en">Creates an <see cref="AncientDialogue" /> without invoking its constructor, which requires at least one line, then assigns an empty <see cref="AncientDialogue.Lines" /> collection and <see cref="ArchitectAttackers.None" /> to its attacker fields.</para>
        ///     <para xml:lang="zh-CN">在不调用构造函数（其要求至少一行台词）的情况下创建 <see cref="AncientDialogue" />，随后将 <see cref="AncientDialogue.Lines" /> 设为空集合，并将攻击者字段设为 <see cref="ArchitectAttackers.None" />。</para>
        /// </summary>
