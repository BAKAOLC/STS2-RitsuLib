    ...
    internal sealed partial class NMainMenuScroller
    {
        ...
        private (float topWeight, float bottomWeight, float start) GetEdgeScaleContext()
        {
            ...
            return (Mathf.SmoothStep(0f, EdgeZone, _visualScroll),
                Mathf.SmoothStep(0f, EdgeZone, ScrollLimit - _visualScroll),
                start);
        }
        ...
        private void ApplyEdgeScale(Control item, float top, float height, float topWeight, float bottomWeight)
        {
            ...
            if (topWeight > 0f && top < EdgeZone)
                BlendEdge(topWeight, top + height * 0.5f, height * 0.5f, height, ref amount, out pivotY);
            if (bottomWeight > 0f && top + height > Size.Y - EdgeZone)
                BlendEdge(bottomWeight, Size.Y - (top + height * 0.5f), pivotY, 0f, ref amount, out pivotY);
        ...
        private static void BlendEdge(
            float weight, float distance, float fromPivot, float toPivot, ref float amount, out float pivotY)
        {
